...
def split_law_for_person(all_laws, criminals_info):
    ...
    for name in criminals_info:
        name_dict[name] = []
        states = []
        for l in criminals_info[name]['accu_law_states']:
            for _ in l[2]:
                states.append(_)
        states = list(set(states))
        for x in states:
            name_dict[name].append(str(get_state_law_mapper()[x + 1]))

        accus = [l[0] for l in criminals_info[name]['accu_law_states']]
        for accu in accus:
            ...
            if type(law_) == str:
                law_ = [law_]
            law_ = [str(hanzi_to_num(x.replace('第', '').replace('条', ''))) for x in law_]
            name_dict[name].extend(law_)
...
def get_str_from_relations(relations):
    ...
    for relation in relations:
        ...
        dict_accu_relation[relation[0]].append('[被告' + relation[1] + ']' + relation[2] + '了' + '[被告' + relation[3] + ']')
    relation_str_list = []
...
def get_str_from_states(criminals_info, name):
    tuple_list = criminals_info[name]['accu_law_states']
    states_set = set()
    for tuple_e in tuple_list:
        for x in tuple_e[2]:
            states_set.add(str(x))
    ...
    for name in criminals_info:
        ...
        for tuple_e in tuple_list:
            for x in tuple_e[2]:
                if int(x) == 0:
                    continue
                states_list.append(str(x))

        info_list.append([len(re.findall('<', name_law_dict[name])), len(criminals_info[name]['accu_law_states']), len(relations_by_name), len(states_list)])

    return info_list



def prepare_complete_data(line, elements='1234'):
    fact, articles, criminals_info, view, article_contents, relations = get_elements_from_line(line)
    input_datas, targets = [], []
    facts = []
    # PROMPT 1 START
    ...
    for name in criminals_info:
        ...
        targets.append(
            '此被告涉及的法条为{}，因此罪名为{}，刑期为{}。'.format(
                name_law_dict[name],
                ','.join([l[0] for l in criminals_info[name]['accu_law_states']]),
...
def prepare_case4_data(line):
    ...
    for name in criminals_info:
        ...
        facts.append(split_fact_for_transform(fact))
        targets.append('此被告罪名为{}。'.format(','.join([l[0] for l in criminals_info[name]['accu_law_states']])))
...
def get_accusation_law_mapper():
    return {'诈骗罪': '第二百六十六条', '合同诈骗罪': '第二百二十四条', '保险诈骗罪': '第一百九十八条',
            ...
            '妨害公务罪': '第二百四十二条', '非法拘禁罪': '第二百三十八条', '敲诈勒索罪': '第二百七十四条',
